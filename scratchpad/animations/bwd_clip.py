...
class BwdClip(manim.MovingCameraScene):
    def construct(self):
        ax = manim.Axes(
            x_range=[-2, 2, 1],
            y_range=[-2, 2, 1],
        ...
        b_display = manim.DecimalNumber(b_tracker.get_value(), num_decimal_places=2).next_to(manim.ORIGIN, manim.LEFT*2).shift(manim.UP * 2)
        b_label = manim.Text("b =", font="Simple Nerd Font", font_size=24).next_to(b_display, manim.LEFT)

        e_display = manim.DecimalNumber(e_tracker.get_value(), num_decimal_places=2).next_to(manim.ORIGIN, manim.LEFT*2).shift(manim.UP * 1)
...
if __name__ == "__main__":
    ...
    os.system("manim -ql --resolution 960,1080 bwd_clip.py BwdClip")
