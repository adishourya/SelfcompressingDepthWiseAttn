...
class QConvT(torch.nn.Module):
    def __init__(self,
        ...
        # He-style init range
        weight_scale = 1 / torch.sqrt(in_channels * out_channels * out_channels)
        self.weight = torch.ones(in_channels, out_channels // groups, kernel_size, kernel_size)
        self.weight = self.weight.uniform_(-weight_scale, weight_scale)

        self.exp_bit = torch.ones(in_channels, 1, 1, 1) * e  # per input channel (acts like filters)
        self.depth_bit = torch.ones(in_channels, 1, 1, 1) * b
    ...
    def _quantized_weight(self):
        ...
        x_upscaled = self.weight / torch.exp2(self.exp_bit)
        half = torch.exp2(b - 1)
        x_clipped = torch.clip(x_upscaled, -half, half - 1)
        x_round = steRound(x_clipped)
        return torch.exp2(self.exp_bit) * x_round
...
if __name__ == '__main__':
    ...
    layer = QConvT(16, 8, kernel_size=3, stride=2, padding=1, output_padding=1)
